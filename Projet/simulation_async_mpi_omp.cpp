...
void simulation(bool affiche)
{
    ...
    int rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);   //rang tu communicateur global

    //paramétrisation de color et key pour le split selon le rang du proc
    int color;
    int key;
    ...
    MPI_Comm localComm;   //création d'un nouveau communicateur 
    MPI_Comm_split(MPI_COMM_WORLD, color, key, &localComm);

    int localrank;
    MPI_Comm_rank(localComm, &localrank);   //obtenir le rang dans le nouveau communicateur
    ...
    if (color==0)
    {
        ...
        while(!quitting)
        {
            ...
            MPI_Send(&go, 1, MPI_INT, 1, tag, MPI_COMM_WORLD); //indique au proc 1 qu'il attend un message

            MPI_Recv(&s, 1, MPI_INT, 1, 0, MPI_COMM_WORLD, &status);
            MPI_Recv(&largeur_grille, 1, MPI_INT, 1, 1, MPI_COMM_WORLD, &status);
            MPI_Recv(&hauteur_grille, 1, MPI_INT, 1, 2, MPI_COMM_WORLD, &status);
            std::vector<épidémie::Grille::StatistiqueParCase> statistiques;
            statistiques.reserve(s);
            MPI_Recv(statistiques.data(), 3*s, MPI_StatParCase, 1, 3, MPI_COMM_WORLD, &status);
            MPI_Recv(&jours_écoulés, 1, MPI_INT, 1, 4, MPI_COMM_WORLD, &status);
    ...
    //####################################
    //simulation par les processus 1 et +
    //####################################
    ...
    if (color==1)
    {
        ...
        std::size_t pop_par_proc = contexte.taux_population/(nbp-1); //population gérée par processus, on suppose que nbp-1 divise taux_population
        ...
        {
            std::default_random_engine motor(100*(i+1+localrank*pop_par_proc));
        ...
        {   
            ...
            if (jours_écoulés%365 == 0)// Si le premier Octobre (début de l'année pour l'épidémie ;-) )
            {
                ...
                for ( int ipersonne = nombre_immunisés_grippe; ipersonne < int(pop_par_proc); ++ipersonne )
                {
            ...
            //on rassemble les données pour toute la grille
            std::vector<épidémie::Grille::StatistiqueParCase> reduc = grille.getStatistiques();
            auto& statistiques = grille.getStatistiques();
            MPI_Allreduce(reduc.data(), statistiques.data(), 3*largeur_grille*hauteur_grille, MPI_INT, MPI_SUM, localComm);


            #pragma omp parallel for schedule(static) reduction(+:compteur_grippe, compteur_agent, mouru)
            ...
            if(localrank == 0)  //on n'envoie qu'une fois au processus 0
            {
                if(flag)
                {
                    MPI_Recv(&go, 1, MPI_INT, 0, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
                    MPI_Send(&s, 1, MPI_INT, 0, 0, MPI_COMM_WORLD);
                    MPI_Send(&(grille.dimension()[0]), 1, MPI_INT, 0, 1, MPI_COMM_WORLD);
                    MPI_Send(&(grille.dimension()[1]), 1, MPI_INT, 0, 2, MPI_COMM_WORLD);
                    MPI_Send(statistiques.data(), statistiques.size(), MPI_StatParCase, 0, 3, MPI_COMM_WORLD);
                    MPI_Send(&jours_écoulés, 1, MPI_INT, 0, 4, MPI_COMM_WORLD);
                }
            }
