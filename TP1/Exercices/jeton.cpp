...
int main( int nargs, char* argv[] )
{
	...
	if (rank==0) {
		...
		MPI_Recv(&msg_to_recv, 1, MPI_INT, nbp-1, tag, MPI_COMM_WORLD, &status);
		std::cout << "0 received " << msg_to_recv << " from " << nbp-1 << "\n" ;
	} else {
		MPI_Recv(&msg_to_recv, 1, MPI_INT, rank-1, tag, MPI_COMM_WORLD, &status);	
		msg_to_send = msg_to_recv + 1;
		std::cout << rank << " sends " << msg_to_send << " to " << (rank+1) % nbp << "\n" ;	
